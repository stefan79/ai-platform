...
KAFKA_DOMAIN_CHANGES_TOPIC="${KAFKA_DOMAIN_CHANGES_TOPIC:-ai-platform-domain-changes}"
...
DYNAMODB_SNAPSHOTS_TABLE="${DYNAMODB_SNAPSHOTS_TABLE:-ai-platform-snapshots}"
...
if ! run_with_timeout 5 docker exec "$REDPANDA_CONTAINER" rpk topic describe "$KAFKA_DOMAIN_CHANGES_TOPIC" \
  --brokers "$KAFKA_BROKERS" >/dev/null 2>&1; then
  run_with_timeout 5 docker exec "$REDPANDA_CONTAINER" rpk topic create "$KAFKA_DOMAIN_CHANGES_TOPIC" \
    --brokers "$KAFKA_BROKERS" \
    --partitions "$KAFKA_PARTITIONS" \
    --replicas "$KAFKA_REPLICATION"
fi
...
if ! aws dynamodb describe-table \
  ...
  --cli-connect-timeout "$AWS_CLI_CONNECT_TIMEOUT" \
  --cli-read-timeout "$AWS_CLI_READ_TIMEOUT" \
  --table-name "$DYNAMODB_SNAPSHOTS_TABLE" >/dev/null 2>&1; then
  aws dynamodb create-table \
    --endpoint-url "$DYNAMODB_ENDPOINT" \
    --region "$DYNAMODB_REGION" \
    --cli-connect-timeout "$AWS_CLI_CONNECT_TIMEOUT" \
    --cli-read-timeout "$AWS_CLI_READ_TIMEOUT" \
    --table-name "$DYNAMODB_SNAPSHOTS_TABLE" \
    --attribute-definitions AttributeName=pk,AttributeType=S AttributeName=sk,AttributeType=S \
    --key-schema AttributeName=pk,KeyType=HASH AttributeName=sk,KeyType=RANGE \
    --billing-mode PAY_PER_REQUEST >/dev/null
fi
...
echo "Redpanda ready. Topics '$KAFKA_EVENTS_TOPIC', '$KAFKA_COMMANDS_TOPIC', '$KAFKA_OUTBOX_TOPIC', '$KAFKA_DOMAIN_CHANGES_TOPIC', '$KAFKA_DEAD_LETTER_TOPIC' are available on $KAFKA_BROKERS."
echo "DynamoDB ready. Tables '$DYNAMODB_DOMAIN_TABLE', '$DYNAMODB_SNAPSHOTS_TABLE', and '$DYNAMODB_OUTBOX_TABLE' are available at $DYNAMODB_ENDPOINT."
echo "Seeded thread snapshot in '$DYNAMODB_SNAPSHOTS_TABLE' with THREAD_ID=$THREAD_ID USER_ID=$USER_ID."
